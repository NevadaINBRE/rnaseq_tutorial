...
We will be working with one main dataset, a small mitochondrial RNASeq toy dataset. There are two other example datasets that correspond to actual data that you can analyze on your own time:

 * toy dataset: RNAseq of mice mitochondrial mRNA
 	* 8 samples : 4 in group A and 4 in group B
 	* single-end, 100bp reads
 	* on the cluster: `/data/gpfs/assoc/biomarker_hunt/data/mouseMT`
...
On the cluster, ensure you are in your personal mouseMT folder.

```sh
cd /data/gpfs/assoc/biomarker_hunt/users/$USER/mouseMT
```
...
!!! note
    ...
    The number is followed by a character which depends on the type of file/folder:
 ...
 * download one of the html report of the mouseMT dataset to your local computer, as well as the one you can find in:
`/data/gpfs/assoc/biomarker_hunt/data/Solutions/Liu2015/010_d_fastqc/SRR3180538_TAM1_1_fastqc.html`
	**OR**
	Download them:
...
!!! note "Reminder"
	 ...
	 to get the data from the remote server to your machine, you may use an SFTP client (Cyberduck), or the command line tool from your machine :	`scp login@xx.xx.xx:path/to/file.txt .`
...
??? success "Alternative sbatch script using array job"
	...
	First, have a file named `fastqFiles.txt` containing the sample fastq file names :
	...
	fastqc -o 010_d_fastqc_array/ $fastqFILE
	```
...
??? success "mouseMT MultiQC sbatch script"
	...
	#SBATCH --cpus-per-task=1
	#SBATCH --mem=1G
	#SBATCH -o 020_l_multiqc_mouseMT.%j.o
	#SBATCH -e 020_l_multiqc_mouseMT.%j.e
	#SBATCH --account=cpu-s5-biomarker_hunt-0
	#SBATCH --partition=cpu-core-0
	...
	```
	On the cluster, this script is also in `/data/gpfs/assoc/biomarker_hunt/data/Solutions/mouseMT/020_s_multiqc.sh`
